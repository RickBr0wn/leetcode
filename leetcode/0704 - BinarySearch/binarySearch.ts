 ...
 **/
const binarySearch = (nums: number[], target: number, minimum = 0, maximum = nums.length - 1): number => {
  const middleIndex = Math.floor((maximum - minimum) / 2 + minimum)

  if (maximum <= minimum && nums[middleIndex] !== target) return -1
